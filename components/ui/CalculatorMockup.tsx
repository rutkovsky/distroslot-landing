export default function CalculatorMockup() {
  const clusters = [
    ...
    { name: "Остальные (4)", qty: 45, pct: 5 },
  ];

  return (
    <div className="overflow-hidden rounded-xl border border-border bg-card shadow-2xl shadow-secondary/5">
      {/* Title bar */}
      <div className="flex items-center gap-2 border-b border-border px-4 py-3">
        <div className="h-3 w-3 rounded-full bg-danger/60" />
        <div className="h-3 w-3 rounded-full bg-yellow-500/60" />
        <div className="h-3 w-3 rounded-full bg-success/60" />
        <span className="ml-3 text-xs text-muted">Результат расчёта</span>
      </div>

      {/* Config */}
      <div className="grid grid-cols-3 gap-3 border-b border-border p-4">
        <div>
          <div className="mb-1 text-[10px] text-muted uppercase tracking-wider">Кабинет</div>
          <div className="rounded-lg border border-border bg-background px-3 py-1.5 text-xs text-foreground">
            Ozon Main Store
          </div>
        </div>
        <div>
          <div className="mb-1 text-[10px] text-muted uppercase tracking-wider">Период анализа</div>
          <div className="rounded-lg border border-border bg-background px-3 py-1.5 text-xs text-foreground">
            14 дней
          </div>
        </div>
        <div>
          <div className="mb-1 text-[10px] text-muted uppercase tracking-wider">Оборачиваемость</div>
          <div className="rounded-lg border border-border bg-background px-3 py-1.5 text-xs text-foreground">
            21 день
          </div>
        </div>
